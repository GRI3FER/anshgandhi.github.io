...
# These sprites are "stacked" to create depth without a 3D engine
...
class Lane:
    def __init__(self, index, type, score):
        ...
        self.type = type
        self.objects = []
        # Speed logic based on lane index
    ...
    def spawn_obj(self, x=None):
        ...
        # Rect represents the "Hitbox" on the ground
...
def reset_game():
    ...
    return {
        "p_world": [WIDTH//2, 12], # [X pixel, Y lane index]
        "lanes": lanes,
...
running = True
while running:
    screen.fill((100, 150, 100)) # Background buffer
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and not state["over"]:
            ...
            if event.key in [pygame.K_DOWN, pygame.K_s] and state["p_world"][1] < state["max_y"] + 5:
                state["p_world"][1] += 1
            if event.key in [pygame.K_LEFT, pygame.K_a]: state["p_world"][0] -= 60
            if event.key in [pygame.K_RIGHT, pygame.K_d]: state["p_world"][0] += 60
